...
- **Optional interfaces**: `SubModuler` (declare children), `DeviceMover` (non-parameter tensors), `Detachable` (state gradient chains), `Resettable`, `Traced`, `TrainToggler`, `NamedInputModule`, `RefValidator`
- **Module utilities**: `CollectParameters` (recursive, deduplicated), `WalkModules` (generic tree walker)
...
- **Test coverage**: 470 tests including numerical gradient checks (autograd ops,
