...
//	Split, Merge, Also, Loop.For, Loop.While, Loop.Until,
...
//	Split(block, block) → Mean()                   Graph-as-Module branches
//	Also(Linear 8→8)                               residual: input + f(input)
//	Through(Dropout(0.1))
//	Loop(block).For(2)                             Tag("refined"), Graph-as-Module body
//	Gate(SoftmaxRouter, Linear, Linear)             .Using("input")
//	Switch(heavyPathSelector, Linear, block)          .Using("refined")  user-defined selector
...
func BuildShowcase() (*graph.Graph, error) {
	...
	return graph.From(nn.MustLinear(2, h)).Tag("input").                             // From + Tag
		...
		Split(block(h), block(h)).Merge(graph.Mean()).                           // Split + Merge (Graph-as-Module)
		Also(nn.MustLinear(h, h)).                                               // Also (residual)
		Through(nn.NewDropout(0.1)).                                             // regularization
		Loop(block(h)).For(2).Tag("refined").                                    // Loop.For (Graph-as-Module body)
		Gate(graph.SoftmaxRouter(h, 2), nn.MustLinear(h, h), nn.MustLinear(h, h)).Using("input"). // Gate + backward ref
		Switch(&heavyPathSelector{}, nn.MustLinear(h, h), block(h)).Using("refined"). // Switch + user-defined selector
